...
- Draws coverage % and an auto WC tag (WC4..WC7) at the top-left.
"""
...
OUTPUT_DIR = "/home/erik/Desktop/Fux/out/v3"


def categorize_wc(coverage_pct: float) -> str:
    """
    Map coverage percentage to WC category:
      - WC4: < 10%
      - WC5: 10% .. 50% (inclusive)
      - WC6: 90% .. < 99%
      - WC7: >= 99% .. 100%
    Anything else (e.g., 50–90%) -> 'WCx' (undefined band for now).
    """
    c = max(0.0, min(100.0, coverage_pct))  # clamp just in case
    if c < 10.0:
        return "WC4"
    if 10.0 <= c <= 50.0:
        return "WC5"
    ...
    if c >= 99.0:
        return "WC7"
    return "WCx"  # undefined band (50–90%)
...
def detect_coverage_noresize(
    ...
    V = hsv[:, :, 2]                                                            #FEKETE VEZETÉK
    V_blur = cv2.GaussianBlur(V, (7, 7), 0)

    Vin = 255 - V_blur
    _, th = cv2.threshold(Vin, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)    #OTSU
    ...
    gray = cv2.cvtColor(bgr, cv2.COLOR_BGR2GRAY)                   #cvtColor itt (gaussian blur)
    ...
    cv2.watershed(grad_rgb, markers)                                                                #watershed itt
    ...
    cv2.drawContours(overlay, contours, -1, (0, 0, 255), thickness=2)                             #körvonalak itt
